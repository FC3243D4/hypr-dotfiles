...
nvidiaCard=$(grep -rl '0x10de' /sys/class/drm/card*/device/vendor 2>/dev/null | head -1 | xargs dirname)
amdCard=$(for v in /sys/class/drm/card*/device/vendor; do
    [[ $(cat "$v" 2>/dev/null) == "0x1002" ]] || continue
    dir=$(dirname "$v")
    echo "$(cat ${dir}/pp_dpm_sclk 2>/dev/null | wc -l) $dir"
done | sort -rn | head -1 | awk '{print $2}')

if [[ -n "$nvidiaCard" ]]; then
    case "$1" in
