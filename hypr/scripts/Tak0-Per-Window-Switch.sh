...
# Read layouts from Lua config.
# Matches:  kb_layout = "it,us"
if ! grep -q 'kb_layout' "$CFG_FILE"; then
    echo "Error: cannot find kb_layout in $CFG_FILE" >&2
    exit 1
fi

raw_layouts=$(grep -oP 'kb_layout\s*=\s*"\K[^"]+' "$CFG_FILE" | head -n1)
if [ -z "$raw_layouts" ]; then
    echo "Error: could not parse kb_layout value from $CFG_FILE" >&2
    exit 1
fi

IFS=',' read -r -a kb_layouts <<< "$raw_layouts"
count=${#kb_layouts[@]}
